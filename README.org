...
*** Evoluzione della stringa
    La funzione principale che solleva diverse congetture, è l'/evoluzione/ di una stringa modulare. E' definita dalla funzione matematica \((s_i) = (s_i + s_{i+1}) \text{ `mod` length}(s).\)
    #+begin_quote
